		...
		*/
		...
		createP13NColumnForAction: function (iContext, oDataField) {
			...
			if (oDataField && oDataField.Value && oDataField.Value.Path) {
				var sP13N = '\\{"columnKey":"' + sColumnKey + '", "sortProperty":"' + oDataField.Value.Path + '", "filterProperty":"' + oDataField.Value.Path + '", "columnIndex":"' + iColumnIndex + '", "actionButton":"true" \\}';
			} else {
		...
		createP13NColumnForContactPopUp: function (oInterface, oContextSet, oDataField, oDataFieldTarget, sAnnotationPath) {
			...
			if (oDataFieldTarget  && oDataFieldTarget.fn && oDataFieldTarget.fn.Path) {
				sP13N += '", "sortProperty":"' + oDataFieldTarget.fn.Path; // for Sort Property
				sP13N += '", "filterProperty":"' + oDataFieldTarget.fn.Path; // for Filter Property
			}
		...
		getColumnListItemTypeForDetailPage : function(oListEntitySet, aSubPages, oManifest, oManifestSettings, bIsDraftEnabled, sAnnotationPath) {
			...
			var sRouteComponentName = oManifestSettings && oManifestSettings.routeConfig && oManifestSettings.routeConfig.component.name;
			...
			if (sOutboundTarget) { // this is the case for the external navigation for which hideChevronForUnauthorizedExtNav (manifest flag) is set to true
				...
				if (bIsDraftEnabled || (!bIsDraftEnabled && sRouteComponentName == "sap.suite.ui.generic.template.ListReport")) {
				...
				// in ListReport with different entitySets in table tabs mode we have to disable the internal navigation for the time being; it should be implemented later as a separate BLI
				if (oAnnotationHelper.fnIsInternalNaviForDiffEntitySets(oListEntitySet.name, sRouteConfigName, aSubPages, oManifestSettings)) {
					return 'Inactive';
				}
		...
		// in ListReport with different entitySets in table tabs mode we have to disable the internal navigation for the time being; it should be implemented later as a separate BLI
		fnIsInternalNaviForDiffEntitySets: function(oListEntitySetName, sRouteConfigName, aSubPages, oManifestSettings) {
			if (sRouteConfigName === "root") {
				var oVariants = oManifestSettings.quickVariantSelectionX && oManifestSettings.quickVariantSelectionX.variants;
				if (!oVariants) { // if it is not a table tabs mode - quit
					return false;
				}
				for (var i in aSubPages) {
					for (var j in oVariants) {
						if (oListEntitySetName == aSubPages[i].entitySet && aSubPages[i].entitySet == oVariants[j].entitySet) {
							if (aSubPages[i].navigation) { // this is the external navigation and it should be enabled in case of different entitySets
								return false;
							} else {
								return true; // the internal navigation should be disabled in case of different entitySets
							}
						}
					}
				}
		...
		getDataFieldLabel: function(oDataFieldValue, oDataField) {
			var sResult;
			if (oDataField.Label) {
				return oDataField.Label.String;
			} else {
