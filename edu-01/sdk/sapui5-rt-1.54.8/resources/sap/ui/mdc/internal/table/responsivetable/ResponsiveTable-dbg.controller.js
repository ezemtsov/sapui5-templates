...
sap.ui.define([
	...
	"../Table.controller"
], function (ResourceModel, TableController) {
	"use strict";
	...
	ResponsiveTableController.prototype.setSelectionMode = function (sSelectionMode) {
		var oToolbar = this.oInnerTable.getHeaderToolbar();
		var aToolbarActions;
		var iMultiplicityTo;
		if (oToolbar) {
			aToolbarActions = this.getToolbarActions(oToolbar.getContent());
	...
	ResponsiveTableController.prototype.enableDisableActions = function () {
		var oToolbar = this.oInnerTable.getHeaderToolbar();
		var aToolbarActions, iSelected;

		if (oToolbar){
			aToolbarActions = this.getToolbarActions(oToolbar.getContent());
			iSelected = this.oInnerTable.getSelectedContexts().length;
	...
	ResponsiveTableController.prototype.handleDataReceived = function (oEvent) {
		...
		if (oError) {
			...
			this.oInnerTable.setNoDataText(ResourceModel.getText("table.TECHINCAL_ERROR"));
			this.oTable.fireShowError(oEvent);
		} else {
			this.oInnerTable.setNoDataText(ResourceModel.getText("table.NO_DATA_TEXT"));
		}
