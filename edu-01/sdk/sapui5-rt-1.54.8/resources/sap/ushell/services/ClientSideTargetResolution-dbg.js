...
sap.ui.define([
    ...
    /**
     * Checks if inplace navigation for legacy technology is configured
     *
     * @param {string} sTechnology
    ...
    ClientSideTargetResolution.prototype.isInPlaceConfiguredFor = function(sTechnology) {
        var oEnableInPlaceForClassicUIs = jQuery.sap.getObject("config.enableInPlaceForClassicUIs", undefined, this._oServiceConfiguration);
        if(!oEnableInPlaceForClassicUIs) {
            return false;
        }
        return oEnableInPlaceForClassicUIs[sTechnology] ? true : false;
    }
    ...
    ClientSideTargetResolution.prototype._mixAppStateIntoResolutionResultAndRename = function(oMatchingTarget, oAppStateService) {
        ...
        function fnMergeAppState(oNewAppState0) {
            ...
            if (!oChangeRecorder.changed && !oChangeRecorder.deleted && !hasAppStateMembers(oMatchingTarget)) {
     ...
     * @private
     */
    ClientSideTargetResolution.prototype._buildNativeWebGuiURI = function(sTcode, sSapSystem) {
        var sUrl, oURI;
        sUrl = "/gui/sap/its/webgui?%7etransaction=" + sTcode + "&%7enosplash=1";
        oURI = new URI(sUrl);
        return this._spliceSapSystemIntoURI(oURI, S_LOCAL_SYSTEM_ALIAS, sSapSystem, "NATIVEWEBGUI", O_SYSTEM_ALIAS_SEMANTICS.apply);
    };
    ...
    ClientSideTargetResolution.prototype._buildWdaURI = function(sApplicationId, sConfigId, oOtherParameters) {
        ...
        return this._spliceSapSystemIntoURI(oURI, S_LOCAL_SYSTEM_ALIAS, sSapSystem, "WDA", O_SYSTEM_ALIAS_SEMANTICS.apply);
    };
    ...
    ClientSideTargetResolution.prototype._stripURI = function(oURI, sSystemAlias, sURIType) {
        ...
        this._resolveSystemAlias(sSystemAlias)
    ...
    /**
     * Resolves the system alias using the adapter. It rejects the
     * promise in case the adapter does not implement the
     * 'resolveSystemAlias' method.
     *
     * @param {string} sSystemAlias
     *   The system alias to be resolved.
     *
     * @returns {jQuery.Deferred.promise}
     ...
     * @private
     */
    ClientSideTargetResolution.prototype._resolveSystemAlias = function (sSystemAlias) {
     ...
     * @private
     */
    ClientSideTargetResolution.prototype._spliceSapSystemIntoURI = function(oURI, sSystemAlias, sSapSystem, sURIType, sSystemAliasSemantics) {
        var oDeferred = new jQuery.Deferred(),
            that = this;
        ...
        if (sSystemAliasSemantics === O_SYSTEM_ALIAS_SEMANTICS.applied && ( // assuming system alias was already applied...
            typeof sSapSystem === "undefined"  // ... no sap system to apply
        ...
        (new Promise(function (fnResolve) {
            ...
            that._stripURI(oURI, sSystemAlias, sURIType)
                .fail(function(sErrorMessage) {
                    oDeferred.reject(sErrorMessage);
                })
            ...
            that._applyAliasToURI(oApply.alias, oApply.targetUri, sURIType)
                .fail(function(sError) {
                    oDeferred.reject(sError);
                })
    ...
    ClientSideTargetResolution.prototype._constructFullWDAResolutionResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment) {
        ...
        that._buildWdaURI(
            ...
            {  /* oOtherParams */
                "sap-system": [ oInboundResolutionResult.systemAlias ]
            }
            ...
            that._constructWDAURLParameters(oMappedIntentParamsPlusSimpleDefaults, oMatchingTarget.mappedDefaultedParamNames)
                .then(function (sUrlParameters) {
                    ...
                    var oResolutionResult = that._createWDAResolutionResult(oMatchingTarget.inbound, sFinalWDAURL, sSapSystem);
    ...
    ClientSideTargetResolution.prototype._constructFullWebguiResolutionResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment) {
        ...
        this._buildNativeWebGuiURI(oInbound.resolutionResult["sap.gui"].transaction, oResolutionResult.systemAlias).done(function(oURIWithSystemAlias) {
            that._mapParameterNamesAndRemoveObjects(oMatchingTarget);
            that._blendParamsIntoNativeWebGUI(oMatchingTarget, oURIWithSystemAlias);
            if (oMatchingTarget.resolutionResult && oMatchingTarget.inbound && oMatchingTarget.inbound.resolutionResult && oMatchingTarget.inbound.resolutionResult["sap.platform.runtime"]) {
    ...
    ClientSideTargetResolution.prototype._constructWDAResolutionResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment, sBaseUrl) {
        ...
        Promise.all([
            (new Promise(function (fnDone) {
                that._spliceSapSystemIntoURI(oWDAURI, oResolutionResult.systemAlias, sSapSystem, "WDA", oResolutionResult.systemAliasSemantics || O_SYSTEM_ALIAS_SEMANTICS.applied)
                    .fail(oDeferred.reject.bind(oDeferred))
            ...
            var oResolutionResult = that._createWDAResolutionResult(oInbound, sFinalWDAURL, sSapSystem);
     ...
     * @private
     */
    ClientSideTargetResolution.prototype._createWDAResolutionResult = function(oInbound, sFinalWDAURL, sSapSystem) {
        var oResolutionResult = {
    ...
    ClientSideTargetResolution.prototype._constructWebguiNowrapResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment, sBaseUrl) {
        ...
        that._spliceSapSystemIntoURI(oWebguiURI, oResolutionResult.systemAlias, sSapSystem, "WEBGUI", oResolutionResult.systemAliasSemantics || O_SYSTEM_ALIAS_SEMANTICS.applied)
            .fail(oDeferred.reject.bind(oDeferred))
    ...
    ClientSideTargetResolution.prototype._constructNativeWebguiWrapResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment, sBaseUrl) {
        ...
        that._spliceSapSystemIntoURI(oWebguiURI, oResolutionResult.systemAlias, sSapSystem, "NATIVEWEBGUI", oResolutionResult.systemAliasSemantics || O_SYSTEM_ALIAS_SEMANTICS.applied )
            .fail(oDeferred.reject.bind(oDeferred))
            .done(function(oWebguiURI) {
                // Reconstruct the final url
                // ASSUMPTION: there are no relevant parameters in the Webgui url, but only Webgui parameters.
    ...
    ClientSideTargetResolution.prototype._constructURLResolutionResult = function(oMatchingTarget, fnBoundFallback, sFixedHashFragment, sBaseUrl) {
        ...
        (new Promise(function (fnDone) {
            if (that._absoluteUrlDefinedByUser(oURI, oResolutionResult.systemAlias, oResolutionResult.systemAliasSemantics)) {
                fnDone(sBaseUrl);
            } else {
                that._spliceSapSystemIntoURI(oURI, oResolutionResult.systemAlias, sSapSystem, "URL", oResolutionResult.systemAliasSemantics || O_SYSTEM_ALIAS_SEMANTICS.applied)
                    .fail(oDeferred.reject.bind(oDeferred))
                    .done(function(oSapSystemURI) {
                        var sSapSystemUrl = oSapSystemURI.toString();
                        fnDone(sSapSystemUrl);
                    });
            }
        }))
        ...
        .then(function (sUrlWithParameters) {
            ...
            oMatchingTarget.resolutionResult.url = sUrlWithParameters;
            oMatchingTarget.resolutionResult.text = oInbound.title;
