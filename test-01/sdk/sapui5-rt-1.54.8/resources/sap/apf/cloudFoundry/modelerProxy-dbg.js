...
sap.ui.define([
	...
	function ModelerProxy(serviceConfiguration, inject){
		...
		function createConfiguration(configurationData, callback){
			...
			ajax({
				type: "POST",
				url : analyticalConfigurationsUri,
				data: JSON.stringify(requestBody),
				dataType : "json",
				success : function(oData, sStatus, oJqXHR) {
					if (oData && !jQuery.isEmptyObject(oData)) {
		...
		function readApplicationList(callback){
			var uriForReadingApplication = applicationsUri + "?$select=Application,ApplicationName";
			ajax({
				type: "GET",
		...
		function readConfigurationList(callback, filter){
			...
			var uriForReadingConfiguration = textFileAndAnalyticalConfigurationsUri + "?$select=AnalyticalConfiguration,AnalyticalConfigurationName&$filter=" + filter.toUrlParam();
			ajax({
				type: "GET",
		...
		function readAnalyticalConfigurationForExport(analyticalConfigurationId, applicationId, callback) {
			var uri = analyticalConfigurationsUri + "/" + analyticalConfigurationId
		...
		function readSingleConfiguration(analyticalConfigurationId, applicationId, callback) {
			var uriForReadingSingleConfiguration = analyticalConfigurationsUri + "/" + analyticalConfigurationId
		...
		function updateApplication(applicationData, applicationId, callback){
			...
			var uriForUpdateApplication = applicationsUri + '/' + applicationId;
			ajax({
				type: "PUT",
		...
		function updateConfiguration(data, callback){
			...
			var uriForUpdateConfiguration = analyticalConfigurationsUri + '/' + analyticalConfigurationId;
		...
		function importConfiguration(applicationData, callback){
			...
			var uriForUpdateConfiguration = applicationAndAnalyticalConfigurationUri + '/' + analyticalConfigurationId;
		...
		function deleteApplication(applicationId, callback){
			var uriForDeleteApplication = applicationsUri + '/' + applicationId;
			ajax({
				type: "DELETE",
		...
		function deleteConfiguration(analyticalConfigurationId, applicationId, callback){
			var uriForDeleteAnalyticalConfiguration = analyticalConfigurationsUri + '/' + analyticalConfigurationId;
			ajax({
				type: "DELETE",
		 ...
		 */
		this.doChangeOperationsInBatch = function(requestConfigurations, callback, applicationId){
			...
			this._readConfigurationListAndTextFile(applicationId).then(function (serverResponse) {
				...
				this._updateRemoteTextFile(applicationId, 'DEV', textFile).then(function () {
					callback(undefined);
		...
		this._readConfigurationListAndTextFile = function(applicationId){
			var promise = new Promise(function(resolve, reject) {
				var uriForReadAnalyticalConfigurationListAndTextFile = textFileAndAnalyticalConfigurationsUri
					+ '?$select=AnalyticalConfiguration,AnalyticalConfigurationName,SerializedAnalyticalConfiguration'
					+ '&$filter=(Application%20eq%20' + applicationId + ')';
				ajax({
					type: "GET",
		...
		this._updateRemoteTextFile = function(applicationId, textFileLanguage, textFile) {
			var promise = new Promise(function(resolve, reject){
				var uriForUpdateTextPropertyFile = textFilesUri + '/' + applicationId + '/' + textFileLanguage;
				var data = {
		...
		function importVendorConfiguration(applicationId, configurationId, callbackConfirmOverwrite, callbackImport, registerApplicationCreatedOnServer, allVendorConfigurations){
			...
			ajax({
				type : "HEAD",
				url: analyticalConfigurationsUri + "/" + configurationId,
				success : function() {
			...
			function callbackCreateNew(newConfigurationName) {
				var url = vendorAnalyticalConfigurationsUri + "/" + configurationId
		...
		function copyConfigurationToCustomerLayer(applicationId, configurationName, serializedAnalyticalConfiguration, callbackImport) {
			var url = "/sap/apf/design-time/customer-content/v1/AnalyticalConfigurations";
			var data = {
		...
		function importConfigurationFromVendorLayer(configurationId, callbackImport) {
			ajax({
				type :"PUT",
				url : vendorImportToCustomerLayerUri + "/" + configurationId,
		...
		this.readAllConfigurationsFromVendorLayer = function() {
			...
			if (vendorContentList) {
				...
				ajax({
					url : vendorAnalyticalConfigurationsUri + "?$select=Application,ApplicationName,AnalyticalConfiguration,AnalyticalConfigurationName",
