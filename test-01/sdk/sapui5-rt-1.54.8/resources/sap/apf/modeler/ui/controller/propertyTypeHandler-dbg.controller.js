...
(function() {
	...
	function _attachEvents(oPropertyTypeHandlerController, oPropertyTypeView) {
		...
		oPropertyTypeView.attachEvent(oConstants.SETFOCUSONREMOVEICON, oPropertyTypeView.getController().setFocusOnAddRemoveIcons.bind(oPropertyTypeView.getController()));
	...
	function _instantiateSubViews(oController) {
		...
		aPropertiesToBeCreated.forEach(function(oPropertyInformation) {
			var oView = _createView(oController, oPropertyInformation, oController.nCounter);
			aPromises.push(oView.getController().initPromise);
		});
		oController.initPromise = jQuery.when.apply(jQuery, aPromises);
	}
