...
(function() {
	...
	function _yearMonthDayFormatter(map, originalFieldValue) {
		...
		if (originalFieldValue.length === 8) {
			dateObject = sap.apf.utils.convertFiscalYearMonthDayToDate(originalFieldValue);
		} else {
			dateObject = new Date(Date.parse(originalFieldValue));
		}

		if (!isNaN(dateObject.getTime())) {
			var dateFormatter = sap.ui.core.format.DateFormat.getDateInstance({ style : "short" });
