 ...
 */
sap.ui.define(['sap/ui/core/format/DateFormat'], function(uiCoreDateFormat) {
	'use strict';
	...
	var module = {
		 ...
		 */
		createPromise: function(valueToBeResolved) {
			...
			function convertValueToDate(v) {
				...
				if (typeof v === 'string') {
					if (v.substring(0, 6) === '/Date(') {
						...
						return sap.apf.utils.convertFiscalYearMonthDayToDate(v);
					}
					return new Date(v);
				}
				return undefined;
			...
			switch (sType) {
				...
				case "Edm.DateTime":
					if (!sap.apf.utils.formatValue.oDateTimeFormat) {
						sap.apf.utils.formatValue.oDateTimeFormat = uiCoreDateFormat.getDateInstance({
				...
				case "Edm.DateTimeOffset":
					if (!sap.apf.utils.formatValue.oDateTimeOffsetFormat) {
						sap.apf.utils.formatValue.oDateTimeOffsetFormat = uiCoreDateFormat.getDateInstance({
		 ...
		 */
		executeFilterMapping: function(oInputFilter, oMappingRequest, aTargetProperties, fnCallback, oMessageHandler) {
		...
		convertFiscalYearMonthDayToDateString: function(yearMonthDay) {
			return sap.apf.utils.convertFiscalYearMonthDayToDate(yearMonthDay).toUTCString();
		},
				...
				case "Edm.DateTime":
					...
					if (attribute === "Date" && internalValue instanceof Date) {
						...
						if (internalValue.length === 8) {
							dateValue = sap.apf.utils.convertFiscalYearMonthDayToDate(internalValue);
							dateInstance = uiCoreDateFormat.getDateInstance({style: "short"});
