					...
					if (oValue && oValue.hasOwnProperty("low")) {// The data in the model corresponds to low and high Objects
							...
							if (oValue.low instanceof Date) {
								...
								if (bIsDateTimeOffsetField) {
									...
									if (aValue) {
										...
										if (aValue.length === 1) {
											aValue[0] = oDate ? FilterProvider.getDateInUTCOffset(oDate) : FilterProvider.getDateInUTCOffset(new Date(aValue[0]));
										} else if (aValue.length === 2) {
											aValue[0] = oDate ? FilterProvider.getDateInUTCOffset(oDate) : FilterProvider.getDateInUTCOffset(new Date(aValue[0]));

											oDate = oFormatter.parse(aValue[1]);
											aValue[1] = oDate ? FilterProvider.getDateInUTCOffset(oDate) : FilterProvider.getDateInUTCOffset(new Date(aValue[1]));
										}
									}
						...
						if (oValue && oValue.hasOwnProperty("ranges")) { // Check if the data is for an unrestricted filter
							...
							while (iLen--) {
								...
								} else if (oDateFormatSettings && oDateFormatSettings.UTC) {// Check if Date values have
						...
						if (oFieldMetadata && oFieldMetadata.fControlConstructor !== MultiComboBox) { // MCB considers only tokens
