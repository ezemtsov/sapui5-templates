	...
	VariantConverterFrom.prototype._addRangesAccordingMetaData = function(oContent, oFilterBar, oFilterMetaData, aRanges, oControl, sName) {
		...
		var fgetValueTypeCompliant = function(sValue) {
			...
			if (sValue && oFilterMetaData) {
				...
				if ((oFilterMetaData.type === "Edm.DateTimeOffset") || (oFilterMetaData.type === "Edm.DateTime") || (oFilterMetaData.type === "Edm.Time")) {
					...
					if (sValue.indexOf('Z') !== (sValue.length - 1)) {
						...
						jQuery.sap.log.error("Property '" + oFilterMetaData.fieldName + "' was added with zulu time zone information.");
					}
				}
			}
		...
		if (aRanges && aRanges.length > 0) {
			...
			if (oFilterMetaData.filterRestriction === sap.ui.comp.smartfilterbar.FilterType.single) {
				...
				var bSpecialMetadataHandling = (oFilterMetaData.type === "Edm.DateTimeOffset") || oFilterMetaData.isCalendarDate;
				if (oControl && !bSpecialMetadataHandling && ((oControl instanceof MultiComboBox) || (oControl instanceof MultiInput))) {
					for (i = 0; i < aRanges.length; i++) {
